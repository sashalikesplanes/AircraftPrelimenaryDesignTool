...
def get_deltas(df):
    df['deltaWingArea'] = df.wingArea.diff().shift(-1)
    df['deltaFuselageLength'] = df.fuselageLength.diff().shift(-1)
    df['deltaTotalC_D'] = df.totalC_D.diff().shift(-1)
    df['deltaPropulsionMass'] = df.propulsionMass.diff().shift(-1)
    df['deltaRequiredEnergy'] = df.requiredEnergy.diff().shift(-1)
    df['deltaFuelMass'] = df.fuelMass.diff().shift(-1)
    df['deltaTotalMass'] = df.totalMass.diff().shift(-1)
    return df
...
if __name__ == "__main__":
    ...
    #df['deltaFuselageLength'] = df['fuselageLength'].to_numpy()[1:] - df['fuselageLength'].to_numpy()[:-1]
