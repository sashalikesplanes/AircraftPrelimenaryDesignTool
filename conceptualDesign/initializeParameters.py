...
def initializeParameters(params):
    ...
    if params["wingTaperRatio"] > 1 or params["wingTaperRatio"] <= 0:
        raise ValueError(f"Taper should be in the following range: 0 < λ <= 1, currently {params['wingTaperRatio']}")

    params["propEfficiency"] = params["engineEfficiency"] * params["fuelCellEfficiency"]
